...
export default function PanelAdminExplicacionesReal() {
  ...
  const verDetalleTarea = (tarea: Pendiente, actividad: Actividad, usuario: Usuario) => {
    setViewMode('detalles');
  ...
  const exportarACSV = () => {
    ...
    const filas = [];
    filas.push([
      "Usuario ID", "Nombre", "Email", "Fuente", "Actividad ID", 
      "Título Actividad", "Fecha Actividad", "Horario", "Status", 
      "Tarea ID", "Nombre Tarea", "Descripción", "Terminada", 
      "Confirmada", "Duración (min)", "Fecha Creación", "Última Actualización"
    ].join(","));
  ...
  if (error || !datos) return <ErrorScreen onRetry={() => cargarDatosReales(true)} />;

  // Main Content selector
  const MainContent = () => {
    switch (viewMode) {
      case 'dashboard':
        return <DashboardView estadisticas={datos.estadisticas} data={datos.data} />;
      case 'colaboradores':
        return (
          <ColaboradoresView
            datos={datos}
